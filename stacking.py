...
def generate_oof_vgg(
    ...
    for fold, (train_idx, valid_idx) in enumerate(
        ...
        train_dataset = FootDataset(fold_train, IMAGE_DIR, transform)
        valid_dataset = FootDataset(fold_valid, IMAGE_DIR, transform)
        ...
        for epoch in range(OOF_EPOCHS):
            ...
            val_loss = validate_one_epoch(model, valid_loader)
            print(
                f"Fold {fold+1} Epoch {epoch+1} Train {train_loss:.4f} Val {val_loss:.4f}"
            )
        ...
        probs = predict_proba_vgg(model, valid_loader)
        oof_pred[valid_idx] = probs
        if best_loss < best_fold_loss:
            best_fold_loss = best_loss
    return oof_pred, best_fold_loss

# OOF KNN
# ==================================================
def generate_oof_knn(
    train_df
):
    numeric_features = [
        "L",
...
if __name__ == "__main__":
    ...
    print(
        "\nGenerating OOF VGG16..."
    )
    oof_vgg, best_loss = generate_oof_vgg(
        train80_df
    )
    print(
        "\nGenerating OOF kNN..."
    )
    oof_knn = generate_oof_knn(
        train80_df
    )
    ...
    y_meta = train80_df["Label"].values
    ...
    final_vgg, train_losses = train_final_vgg(
        train80_df
    )
    ...
    report_path = os.path.join(
        REPORT_DIR,
        "stacking",
        "bismillah-stacking-crossfold1.txt"
    )
    os.makedirs(
    ...
    with open(
        ...
        f.write(
            f"Training Epochs      : {FINAL_EPOCHS}\n"
        )
        f.write(
    ...
    plt.savefig(
        os.path.join(
            ...
            "stacking",
            "conmat",
            "bismillah-stacking-cm-crossfold1.png"
        )
    )
    plt.close()
    print("Checkpoint B")
    ...
    plt.savefig(
        os.path.join(
            ...
            "stacking",
            "rocurve",
            "bismillah-stacking-roc1-crossfold.png"
        )
    )
    plt.close()
    print("Checkpoint C")
    ...
    plt.savefig(
        os.path.join(
            ...
            "stacking",
            "grafik",
            "bismillah-stacking_loss-crossfold1.png"
        )
    )
    plt.close()
    print("Checkpoint D")
